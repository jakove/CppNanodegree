...
Sudoku::Sudoku(const Sudoku& other_sudoku){
    init_vals_ = other_sudoku.init_vals_;
    board_ = other_sudoku.board_;
}
...
bool Sudoku::IsInitValue(int row, int col)
{
...
bool Sudoku::CheckValue(int val)
{
    return val >= 0 && val <= 9;
}

bool Sudoku::CheckCol(int row, int col, int val)
{
...
bool Sudoku::CheckRow(int row, int col, int val)
{
...
bool Sudoku::CheckBlock(int row, int col, int val)
{
    int block_length = 3;
...
bool Sudoku::CheckIndices(int row, int col)
{
    return row >= 0 && row <= 8 && col >= 0 && col <= 8;
}

bool Sudoku::Delete(int row, int col)
{
    if (!CheckIndices(row, col))
    {
...
const std::array<std::array<int, 9>, 9> &Sudoku::GetBoard()
{
    return board_;
}
...
bool Sudoku::InitValue(int row, int col, int val)
{
    if (Set(row, col, val))
    {
...
bool Sudoku::SaveState(const std::string &path)
{
...
bool Sudoku::LoadState(const std::string &path)
{
...
void Sudoku::Clear()
{
    for (int row_number = 0; row_number < board_.size(); row_number++)
    {
        for (int col_number = 0; col_number < board_[0].size(); col_number++)
        {
            init_vals_[row_number][col_number] = 0;
            board_[row_number][col_number] = 0;
        }
    }
}
